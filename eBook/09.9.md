...
每一个 Git 项目都需要一个对包进行描述的 README.html 文件，所以需要打开你的文本编辑器（gedit、notepad 或 LiteIde）并添加一些说明进去。

- 添加所有文件到仓库：`git add README.html uc.go uc_test.go Makefile`。
...
- [目录](directory.html)
- 上一节：[自定义包的目录结构、go install 和 go test](09.8.html)
- 下一节：[Go 的外部包和项目](09.10.html)
