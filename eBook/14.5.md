...
`time.Tick()` 函数声明为 `Tick(d Duration) <-chan Time`，当你想返回一个通道而不必关闭它的时候这个函数非常有用：它以 d 为周期给返回的通道发送时间，d是纳秒数。如果需要像下边的代码一样，限制处理频率（函数 `client.Call()` 是一个 RPC 调用，这里暂不赘述（参见第 [15.9](15.9.html) 节）：
...
- [目录](directory.html)
- 上一节：[使用select切换协程](14.4.html)
- 下一节：[协程和恢复（recover）](14.6.html)
