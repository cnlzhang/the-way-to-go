...
2）errorhandler：这是一个包装函数。接收一个 fType1 类型的函数 fn 并返回一个调用 fn 的函数。里面就包含有 defer/recover 机制，这在 [13.3 节](13.3.html)中有相应描述。
...
当错误发生时会 recover 并打印在日志中；除了简单的打印，应用也可以用 template 包（参见 [15.7 节](15.7.html)）为用户生成自定义的输出。check() 函数会在所有的被调函数中调用，像这样：
...
我们会在 [15.5 节](15.5.html)的 web 应用中使用这种模式。
...
写一个 ConvertInt64ToInt 函数把 int64 值转换为 int 值，如果发生错误（提示：参见 [4.5.2.1 节](04.5.html#4521-整型-int-和浮点型-float)）就 panic。然后在函数 IntFromInt64 中调用这个函数并 recover，返回一个整数和一个错误。请测试这个函数！

## 链接

- [目录](directory.html)
- 上一节：[自定义包中的错误处理和 panicking](13.4.html)
- 下一节：[启动外部命令和程序](13.6.html)
